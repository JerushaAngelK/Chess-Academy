...
public class offlineCoursesServiceImpl implements offlineCoursesService {
    ...
    @Override
    public offlineCoursesDto updateOfflineCourses(int id, offlineCoursesDto offlineCoursesDto) {
        offlineCourses offlineCourses = offlineCoursesRepository.findById(id)
        .orElseThrow(() -> new ResourceNotFoundException("Offline Courses not found with id : " + id));
        // offlineCourses.setId(offlineCoursesDto.getId());
        offlineCourses.setImage(offlineCoursesDto.getImage());
        offlineCourses.setTitle(offlineCoursesDto.getTitle());
        offlineCourses.setEnroll(offlineCoursesDto.isEnroll());
        
        offlineCourses updatedOfflineCourses = offlineCoursesRepository.save(offlineCourses);
        return offlineCoursesMapper.mapToOfflineCoursesDto(updatedOfflineCourses);
    }
    
    @Override
    public void deleteOfflineCourses(int id) {
        offlineCoursesRepository.deleteById(id);
    }
}
